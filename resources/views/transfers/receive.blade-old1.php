...
<form action="{{ route('transfers.receive.store') }}" method="POST" 
    ...
    @if (isset($available_items))    
        <section class="table-responsive-md rounded mt-3">
            <table class="table table-bordered align-middle" id="receive" style="min-width: 50rem;">
                <thead class="small">
                    <tr>
                        <th style="min-width: 10rem;">Item</th>
                        <th style="min-width: 8rem;">Avaiable Quantity (Kg)</th>
                        <th>Item To Receive</th>
                        <th style="min-width: 6rem;">Rate</th>
                        <th style="min-width: 10rem;">Receive Quantity (Current)</th>
                        <th style="min-width: 8rem;">Receive Quantity <br>(+ Waste)</th>
                        <th style="min-width: 6rem;">Waste %</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($available_items as $available_item)
                    <?php $data = DB::table('opening_stock')->where('item_name',@$available_item->item_id)->first();
                    
                    ?>

                    <tr>
                        <td>{{ $available_item->item_name }}</td>
                        <td>{{ $available_item->total_quantity + @$data->quantity }}</td>
                        <td style="min-width: 14rem;" class="text-start">
                        ...
                        <td>
                            <div class="input-group">
                                ...
                                <input type="number" name="receive_quantities[]" step="0.01" class="form-control" 
                                 max="{{ $available_item->total_quantity }}"
