    ...
    @if (isset($getItem))    
        <section class="table-responsive-lg rounded mt-3">
            <table class="table table-bordered align-middle" id="example">
                ...
                <tbody>
                    @foreach ($getItem as $available_item)
                    <?php $total_quantity = $available_item->transferTransactions ? $available_item->transferTransactions->quantity : 0;
                    $amount = $available_item->quantity;
                    $plusrecord = $amount + $total_quantity;
                     ?>
...
<script>
  ...
  $(document).ready(()=>{
    ...
    // check stock
    $('table#transfer tbody').on('change',`input[name='send_quantities[]']`,function(){
